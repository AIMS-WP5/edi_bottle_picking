...
/** \brief The named SRDF poses (``<group_state>`` in
    ...
    they put the tool on OPPOSITE SIDES of the robot:
      ...
      "edi"   (default) -- above_box / near_box. The real EDI cell, and equally the
      ...
      "isaac" (legacy)  -- above_box_1 / ai_after_pickup / wait_slam. The edi_isaacsim
                           ...
                           keeps working through the deprecation window. above_box_1 is no
                           longer in the shared edi_ur SRDF: bringup_sim_stack.sh adds it via
                           config/srdf/isaac_legacy_poses.srdf.xacro (extra_srdf).
...
struct ScenarioPoses
{
    ...
    std::string above_box        = "above_box";
    ...
    std::string dp_handoff       = "ai_start";
